...
\draw[color=BrickRed] ($(R3) + (0,3pt)$) -- ($(R3) + (0,-3pt)$);
\draw[color=BrickRed] (R2) -- (R3) node[color=black,midway, above=3pt](L1) {$b$ {\tiny \asset}};
\node[text=BrickRed,xshift=-52pt,yshift=-8pt] at (L1) {flash loan};

\draw[color=MidnightBlue] ($(R0) + (0,3pt)$) -- ($(R0) + (0,-3pt)$);
\draw[color=MidnightBlue] ($(R5) + (0,3pt)$) -- ($(R5) + (0,-3pt)$);
\draw[color=MidnightBlue] (R0) -- (R5) node[color=black,midway, above=3pt](L2) {$z$ {\tiny \stasset}};
