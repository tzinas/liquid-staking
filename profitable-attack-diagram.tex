...
\coordinate[above=45pt] (R0) at (P0);
\coordinate[above=45pt] (R5) at (P5);


\draw[color=myred] ($(R2) + (0,3pt)$) -- ($(R2) + (0,-3pt)$);
\draw[color=myred] ($(R3) + (0,3pt)$) -- ($(R3) + (0,-3pt)$);
\draw[color=myred] (R2) -- (R3) node[color=black,midway, above=3pt](L1) {$b$ {\tiny \asset}};
\node[text=myred,xshift=-52pt,yshift=-8pt] at (L1) {flash loan};

\draw[color=myblue] ($(R0) + (0,3pt)$) -- ($(R0) + (0,-3pt)$);
\draw[color=myblue] ($(R5) + (0,3pt)$) -- ($(R5) + (0,-3pt)$);
\draw[color=myblue] (R0) -- (R5) node[color=black,midway, above=3pt](L2) {$z$ {\tiny \stasset}};
