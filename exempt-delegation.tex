...
Until now, we assumed the adversary taking a $z$ \stasset loan,
needed to short \stasset at $t_0$, without any requirements.
However, in practice, the adversary uses an
initial capital of $u$ \asset as collateral to obtain the
$z = \frac{u}{\gammastasset} \frac{s_0}{b_0}$ \stasset loan.
...
Hence $c + \betaasset b \leq b^*$ and the adversary is bounded to use up to
$b \leq \frac{u}{(\frac{1}{\phi} + \betaasset)\gammastasset}$
to move the price.

Including the flash loan cost, the final profit of the attack is now
...
In a secure liquid staking protocol, in the rational model,
this attack should be unprofitable for the adversary.
To achieve this, the protocol must select the parameter $\frac{\phi}{q}$
in order to satisfy $\alpha \leq 0$. Solving for $\frac{\phi}{q}$ gives:
...
The constants $p$, $\betaasset$, $f$, $b_0$, $\gammastasset$ and $u$ should
be substituted with the appropriate values of the blockchain and market conditions
...
The white area under the black line is where the protocol is secure.

In Figure~\ref{fig:plotf} we consider a blockchain with slashing percentage
...
%TODO: find out if setting $b_0$ here is necessary
%, a liquid staking protocol with $b_0 = 10000$ \asset initial holdings,

%TODO: correct the plots
\begin{figure}[htb]
  \centering
...
%TODO: Move this before equation
% Free money makes the adversary more powerful
...
%TODO: check if the same happens for β and γ
...
Concretely if flash loans are free ($\beta = 0$), interest rates are $r = 0$,
%TODO: why is r used here and not f?
...
\begin{gather*}
  ...
  % TODO: is naming necessary here?
  % TODO: should the previous equation be named asterisk?
\end{gather*}
