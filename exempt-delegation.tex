...
$c$; therefore, she can set $c = \frac{b}{\phi}$.
Instead of requiring the exempt delegation amount as extra assets set aside, we will make them part of
...
$c = \frac{b}{\phi}$ \asset to $\mathcal{V}$ at $t_1$.
Hence $c \leq b^*$ and the adversary may use up to
$b \leq u\phi$ to move the price.


The profit of the attack now becomes $\alpha = b^* - b' - \frac{q}{\phi}b$.
...
Concretely if flash loans are free ($\betaasset = 0$), interest rates are zero ($f = 1$),
and the collateral ratio is $\gammastasset = 100\%$, equation~\eqref{eq:phi-choice} simplifies to:
...
unfair punishment is no longer a rational outcome.
