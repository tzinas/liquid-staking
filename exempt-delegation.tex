...
The parameter $\frac{\phi}{q}$ should be calculated independently for each protocol by replacing
...
and the collateral ratio required to be $\gamma = 100\%$, the above is greatly simplified to:

\begin{gather*}
  \frac{q}{\phi} \geq p \frac{u}{b_0} \label{eq:phi-choice} \tag{$\ast$}
\end{gather*}
...
is slashed. A lower exempt delegation factor $\phi$ also makes the protocol more secure.
A larger exempt delegation $c$ is required to liquid stake the same $b$ \asset.
...
besides the adversary. Equation~\eqref{eq:phi-choice}, with some extra safety
margin, can be used in practice to calculate the appropriate value of $\phi$
...
(a $\frac{u}{b_0} = 10\%$ market domination),
if $p = q = 100\%$ is the slashing rate, then $\phi \ll \frac{b_0}{u} = 10$
