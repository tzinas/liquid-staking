...
In a staking protocol, whether liquid or not, each delegator has an opinion
about which validator they wish to delegate to.

\begin{definition}[Delegation Wish]
  For each delegator, we define
...
\begin{definition}[Wisdom]
  A delegator is \emph{wise} if their delegation wish points to an honest
  validator. Otherwise, the delegator is \emph{unwise}.
\end{definition}
...
\begin{definition}[Proportional Representation]
  ...
  equal to the sum of the proportions of \stasset held by the \stasset holders
  who wish to delegate to that validator.
\end{definition}

In order to achieve this, the process of liquid
...
In a liquid staking protocol with proportional
